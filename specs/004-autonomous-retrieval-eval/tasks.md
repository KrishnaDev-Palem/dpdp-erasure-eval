...
- [ ] T002 [P] Create `core/tools/` package skeleton with empty `core/tools/__init__.py` and a `core/tools/registry.py` stub exporting a `ToolRegistry` Protocol (callable dispatch interface only; full implementation in T022) per plan.md project structure (FR-001, U1 seam dependency)
...
- [ ] T024 [US2] Seed committed cache entries for all location-bearing export subjects × `autonomous` × `sample_index` 0–4 under `cache/primary/autonomous/` (2 subjects × 5 samples = 10 entries; skip `empty-locations-subject` per Feature 002 seeding; additive only; do not edit `export/`) (FR-011, FR-019, SC-003, SC-010)

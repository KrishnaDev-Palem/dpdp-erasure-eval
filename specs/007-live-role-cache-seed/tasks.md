...
- [ ] T005 [P] [US3] Write `tests/gate/test_acceptance_live_role_gate_replay.py`: `run_adversarial_gate_sweep` with `GateSweepConfig(model_id="gemini-3.5-flash", cache_mode="offline")` and call-recording seam; assert completion, `classify_calls == []`, `slice_case_count == 90` with all 5 sample rollups, two-run outcome determinism, result metadata (pattern from `tests/gate/test_acceptance_gate_cache_offline.py`)
...
- [ ] T020 Full merge-gate regression: `uv run pytest -v` with `CACHE_MODE=offline` and all provider keys unset — 100% pass including Features 001–006 regression suites and the four new replay files (007 SC-002; SC-003; FR-013)
...
- [ ] T023 Complete the Feature 006 SC-002 completion checklist in `specs/007-live-role-cache-seed/quickstart.md` (all nine items checked) and update item statuses in `specs/007-live-role-cache-seed/checklists/cache-seeding.md` (SC-001 completes 006 SC-002; SC-005)

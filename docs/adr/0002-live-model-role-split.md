...
adversarial gate is high-volume binary classification over a 90-case slice at N = 5. Section 9 bounds spend and
names a single primary model for the adjudication product run. Section 11 defers a two-model comparison within the
...
4. Concrete model strings are configuration confirmed at wiring time per section 11. This ADR records the role
...
- Gate volume runs on the cheaper classifier consistent with the section 9 spend guardrail.
- The injected model seam already supports swapping either role by configuration, so the section 11 deferred
...
- **Two models within the adjudication ablation** — rejected; this is the section 11 deferred two-configuration
  comparison, out of scope for v1 by the decision log.
- **Hardcoding model strings in this ADR** — rejected; violates the config-not-spec rule in section 11. The ADR
